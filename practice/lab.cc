...
int create(string name) {
    ...
    for(index = 0; index < cur_user.next->cur_user_direct_size; index++) {
        if(path == cur_user.next->direct[index].directname) { break; }
    }
    int i;
    for(i = 0; i < cur_user.next->direct[index].cur_file_size; i++)  //遍历当前目录，查看是否有文件重名
    {
        if(name == cur_user.next->direct[index].ufd[i].filename) break;
    }
...
int open(string name) {
    ...
    {
        if(cur_user.next->direct[index].ufd[k].filename == name) break;
    }
...
int del(string name)  //删除文件打开表的文件数量不用减一，因为文件打开就不能删除文件
{
    ...
    {
        if(cur_user.next->direct[index].ufd[i].filename == name) break;
    }
    if(i >= cur_user.next->direct[index].cur_file_size) {
        cout << "没有这个文件" << endl;
        return -1;
    ...
    {
        if(cur_opentable->uof[j].filename == name) break;
    }
...
int read(string name) {
    ...
    for(index1 = 0; index1 < cur_user.next->cur_user_direct_size; index1++) {
        if(path == cur_user.next->direct[index1].directname) { break; }
    }
    int a;
    for(a = 0; a < cur_user.next->direct[index1].cur_file_size; a++)  //判断文件是否存在
    {
        if(cur_user.next->direct[index1].ufd[a].filename == name) break;
    }
    ...
    for(i = 0; i < cur_opentable->cur_openfilesize; i++) {
        if(cur_opentable->uof[i].filename == name) break;
    }
...
int write(string name, char* buf, int len) {
    ...
    if(cur_opentable->uof[fd].protect_code != 2) {
        ...
        {
            for(i = 0; i < 512 - cur_opentable->uof[fd].pointer % 512; i++) {  //写入一部分的内容到最后一个磁盘块的剩余空间
                first[i] = buf[i];
            }
            ...
            if(need_offset_size > 0) need_block_size++;  //总共需要这么磁盘块
            int unused_block_size = 0;                   //记录没有使用过的磁盘块的个数
            //int first_unused_block; //记录第一个没有被使用过的磁盘
            int flag = 0;
            for(int i = 0; i < 64; i++) {
                if(fat[i].used == 0) { unused_block_size++; }
            }
...
void dir() {
    ...
    } else  //显示目录下的文件
    {
        ...
        {
            cout << "\t" << cur_user.next->direct[index1].ufd[i].filename << "\t" << cur_user.next->direct[index1].ufd[i].protect_code << "\t"
...
int login() {
    ...
    for(i = 0; i < cur_user_size; i++) {
        if(mfd[i].username == name && mfd[i].password == password) { break; }
    }
    if(i >= cur_user_size) {
...
void cd() {
    ...
    {
        if(temp_path == cur_user.next->direct[i].directname) break;
    }
...
void mkdir(string name) {
    ...
    {
        if(cur_user.next->direct[i].directname == name) break;
    }
    if(i < cur_user.next->cur_user_direct_size) {
        cout << "该目录已经存在了" << endl;
        return;
